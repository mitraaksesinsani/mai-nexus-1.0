...
export async function GET(req: NextRequest) {
  try {
    ...
    // 1. Count pending PR approvals (Purchase Request, table: rfcs)
    let prApprovals = 0;
    if (['PROCUREMENT', 'OWNER', 'DIREKTUR', 'ADMIN', 'SUPER_ADMIN'].includes(role)) {
      try {
        const prRes = await pool.query(`
          ...
          AND (
            approval_destination IS NULL 
            OR approval_destination = $1 
            OR $2 IN ('ADMIN', 'SUPER_ADMIN')
          )
        `, [userId, role]);
        prApprovals = parseInt(prRes.rows[0]?.count, 10) || 0;
      } catch {
        const prRes = await pool.query(`
          SELECT COUNT(*) as count 
          FROM rfcs 
    ...
    // 2. Count pending RFC approvals (Request for Consumption, table: consumption_requests)
    let rfcApprovals = 0;
    ...
    // 4. Count ready Material Receives (DOs that are shipping or waiting to be received)
    ...
    // 5. Count pending Logistics (Approved/Processed POs waiting to be made into DOs)
